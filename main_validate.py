...
def validate_model(model: LCRRotHopPlusPlus, dataset: EmbeddingsDataset, name='LCR-Rot-hop++'):
    ...
    print(f"Knowledge injected into layers: {list(knowledge_layers)}")
...
def main():
    ...
    word2vec = gensim.models.Word2Vec.load("data/myontology2015/output")
    ontology = Graph().parse("data/raw/ontology.owl")


    device = torch.device('cuda' if torch.cuda.is_available() else
                          'mps' if torch.backends.mps.is_available() else 'cpu')
    model = LCRRotHopPlusPlus(hops=hops, word2vec=word2vec, ontology=ontology).to(device)
