...
from model.bert_encoder import ontology
...
class LCRRotHopPlusPlus(nn.Module):
    def __init__(self, word2vec, ontology, dropout_prob=0.3, output_size=3, input_size=768, hidden_size=300, hops=3, 
        ...
        self.embeddings_layer = EmbeddingsLayer(device=self.device, dense1=self.dense1, dense2=self.dense2, proj1=self.proj1, proj2=self.proj2, word2vec = self.word2vec, ontology=self.ontology)
    ...
    def forward(self, sentence, left_index: int, right_index: int, knowledge_layers,
        ...
        embedding_data = generate_OneEmbedding(embeddings_layer=self.embeddings_layer, sentence=sentence, target_from=left_index, target_to=right_index, knowledge_layers=knowledge_layers)
        ...
        if representation_left is None:
            representation_left = torch.zeros(self.representation_size, device=self.device)
        if representation_right is None:
            representation_right = torch.zeros(self.representation_size, device=self.device)
